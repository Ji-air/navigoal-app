...
describe("evaluerCaptain", () => {
  it("but en temps réglementaire → true", () =>
    expect(evaluerCaptain([mkBut("p1")], "p1")).toBe(true))
  it("but en prolongations → true", () =>
    expect(evaluerCaptain([mkBut("p1", "prolongations")], "p1")).toBe(true))
  it("but aux tirs au but → false (CL6)", () =>
    expect(evaluerCaptain([mkBut("p1", "tirs_au_but")], "p1")).toBe(false))
  it("but d'un autre joueur → false", () =>
    expect(evaluerCaptain([mkBut("p2")], "p1")).toBe(false))
  it("assist → false (Captain ne déclenche que sur but)", () =>
    expect(evaluerCaptain([mkAssist("p1")], "p1")).toBe(false))
  it("aucun event → false", () =>
    expect(evaluerCaptain([], "p1")).toBe(false))
  it("event officiel=false ignoré (CL5)", () => {
    const e: EvenementMatch = { id: "x", joueurId: "p1", type: "but", phaseJeu: "temps_reglementaire", officiel: false, minutesJouees: 90 }
    expect(evaluerCaptain([e], "p1")).toBe(false)
  })
  it("minutesJouees=0 → false (CL1)", () => {
    const e: EvenementMatch = { id: "x", joueurId: "p1", type: "but", phaseJeu: "temps_reglementaire", officiel: true, minutesJouees: 0 }
    expect(evaluerCaptain([e], "p1")).toBe(false)
  })
})
...
describe("evaluerSecond", () => {
  it("but en temps réglementaire → true", () =>
    expect(evaluerSecond([mkBut("p1")], "p1")).toBe(true))
  it("assist → true", () =>
    expect(evaluerSecond([mkAssist("p1")], "p1")).toBe(true))
  it("pré-assist → true", () =>
    expect(evaluerSecond([mkPreAssist("p1")], "p1")).toBe(true))
  it("but aux tirs au but → false (CL6)", () =>
    expect(evaluerSecond([mkBut("p1", "tirs_au_but")], "p1")).toBe(false))
  it("aucun event → false", () =>
    expect(evaluerSecond([], "p1")).toBe(false))
  it("but d'un autre joueur → false", () =>
    expect(evaluerSecond([mkBut("p2")], "p1")).toBe(false))
})

describe("evaluerNavigator", () => {
  it("pré-assist → true", () =>
    expect(evaluerNavigator([mkPreAssist("p1")], "p1")).toBe(true))
  it("assist → true", () =>
    expect(evaluerNavigator([mkAssist("p1")], "p1")).toBe(true))
  it("but → false (Navigator ne déclenche pas sur but, R2)", () =>
    expect(evaluerNavigator([mkBut("p1")], "p1")).toBe(false))
  it("but aux TAB → false (CL6)", () =>
    expect(evaluerNavigator([mkBut("p1", "tirs_au_but")], "p1")).toBe(false))
  it("aucun event → false", () =>
    expect(evaluerNavigator([], "p1")).toBe(false))
})
...
describe("calculerImpulsionPoste", () => {
  it("poste null (vide) → 0nm", () =>
    expect(calculerImpulsionPoste({ poste: "Captain", joueurId: null, palier: "Boost", evenements: [], butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(0))
  it("participationMinutes=0 → 0nm (CL1)", () =>
    expect(calculerImpulsionPoste({ poste: "Captain", joueurId: "p1", palier: "Boost", evenements: [mkBut("p1")], butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 0 })).toBe(0))
  it("Second : assist déclenche (Boost) → 40nm", () =>
    expect(calculerImpulsionPoste({ poste: "Second", joueurId: "p1", palier: "Boost", evenements: [mkAssist("p1")], butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(40))
  it("Navigator : but ne déclenche pas → 0nm", () =>
    expect(calculerImpulsionPoste({ poste: "Navigator", joueurId: "p1", palier: "Wind", evenements: [mkBut("p1")], butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(0))
})
...
describe("Scénario 1 — J3 journée solide sans bonus diversité", () => {
  ...
  const eventsEgypte: EvenementMatch[] = []
  ...
  it("Captain Mbappé (France, Breeze) → 10nm", () =>
    expect(calculerImpulsionPoste({ poste: "Captain", joueurId: MBAPPE, palier: "Breeze", evenements: eventsFrance, butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(10))

  it("Second Salah (Égypte, Wind) → 0nm (ne marque pas)", () =>
    expect(calculerImpulsionPoste({ poste: "Second", joueurId: SALAH, palier: "Wind", evenements: eventsEgypte, butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(0))

  it("Navigator De Bruyne (Belgique, Breeze) → 10nm (pré-assist)", () =>
    expect(calculerImpulsionPoste({ poste: "Navigator", joueurId: DE_BRUYNE, palier: "Breeze", evenements: eventsBelgique, butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(10))

  it("Watch Militão (Brésil, Breeze) → 10nm (0 but encaissé)", () =>
    expect(calculerImpulsionPoste({ poste: "Watch", joueurId: MILITAO, palier: "Breeze", evenements: eventsBresil, butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(10))

  it("Keeper Alisson (Brésil, Breeze) → 10nm (4 arrêts)", () =>
    expect(calculerImpulsionPoste({ poste: "Keeper", joueurId: ALISSON, palier: "Breeze", evenements: eventsBresil, butsEncaissesNation: 0, arretsDuJoueur: 4, participationMinutes: 90 })).toBe(10))
  ...
  it("Bonus diversité : N=4, 1 outsider (Égypte) < seuil 2 → 0nm", () =>
    expect(calculerBonusDiversite({
...
describe("Scénario 2 — J8 deux postes même nation, sans bonus", () => {
  ...
  it("Captain Al-Dawsari (Arabie Saoudite, Boost) → 40nm", () =>
    expect(calculerImpulsionPoste({ poste: "Captain", joueurId: AL_DAWSARI, palier: "Boost", evenements: eventsSA, butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(40))

  it("Keeper Al-Owais (Arabie Saoudite, Boost) → 40nm (3 arrêts)", () =>
    expect(calculerImpulsionPoste({ poste: "Keeper", joueurId: AL_OWAIS, palier: "Boost", evenements: eventsSA, butsEncaissesNation: 0, arretsDuJoueur: 3, participationMinutes: 90 })).toBe(40))

  it("Collectif Arabie Saoudite (Boost) → 40nm — une seule impulsion R4", () =>
    expect(calculerImpulsionCollective("Boost")).toBe(40))

  it("Bonus diversité : N=1 (1 match terminé) → seuilDiversite(1)=null → 0nm", () =>
    expect(calculerBonusDiversite({
  ...
  it("Score total J8 → 120nm", () =>
    expect(calculerScoreJournee([40, 40, 40], 0)).toBe(120))
})
...
describe("Scénario 3 — J5 défensive minimale, bonus inactif", () => {
  ...
  it("Keeper Ochoa (Mexique, Breeze) → 10nm (3 arrêts)", () =>
    expect(calculerImpulsionPoste({ poste: "Keeper", joueurId: OCHOA, palier: "Breeze", evenements: [], butsEncaissesNation: 0, arretsDuJoueur: 3, participationMinutes: 90 })).toBe(10))

  it("Autres postes Favoris sans event → 0nm chacun", () =>
    expect(calculerImpulsionPoste({ poste: "Captain", joueurId: "cap", palier: "Breeze", evenements: [], butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(0))

  it("Bonus diversité : N=3, 0 outsider (tous Breeze) < seuil 1 → 0nm", () =>
    expect(calculerBonusDiversite({
  ...
  it("Score total J5 → 10nm", () =>
    expect(calculerScoreJournee([10], 0)).toBe(10))
})
...
describe("Scénario 4 — J14 record absolu, bonus maximum", () => {
  ...
  const eventsTurquie: EvenementMatch[] = []

  it("Captain Al-Dawsari (Boost) → 40nm", () =>
    expect(calculerImpulsionPoste({ poste: "Captain", joueurId: AL_DAWSARI, palier: "Boost", evenements: eventsSA, butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(40))

  it("Second Aboubakar (Boost) → 40nm", () =>
    expect(calculerImpulsionPoste({ poste: "Second", joueurId: ABOUBAKAR, palier: "Boost", evenements: eventsCameroun, butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(40))

  it("Navigator Kudus (Boost) → 40nm (pré-assist)", () =>
    expect(calculerImpulsionPoste({ poste: "Navigator", joueurId: KUDUS, palier: "Boost", evenements: eventsGhana, butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(40))

  it("Watch Demiral (Boost) → 40nm (0 but encaissé)", () =>
    expect(calculerImpulsionPoste({ poste: "Watch", joueurId: DEMIRAL, palier: "Boost", evenements: eventsTurquie, butsEncaissesNation: 0, arretsDuJoueur: 0, participationMinutes: 90 })).toBe(40))

  it("Keeper Al-Owais (Boost) → 40nm (4 arrêts)", () =>
    expect(calculerImpulsionPoste({ poste: "Keeper", joueurId: AL_OWAIS, palier: "Boost", evenements: eventsSA, butsEncaissesNation: 0, arretsDuJoueur: 4, participationMinutes: 90 })).toBe(40))

  it("4 collectifs Boost → 4 × 40nm = 160nm", () => {
    const total = ["Boost", "Boost", "Boost", "Boost"].reduce(
      (sum, p) => sum + calculerImpulsionCollective(p as "Boost"),
      0,
    )
    expect(total).toBe(160)
  })

  it("Bonus diversité : N=6, 4 outsiders ≥ seuil 2, 9 impulsions → 90nm", () =>
    expect(calculerBonusDiversite({
...
describe("Scénario 5 — J4 règle d'exclusion R6", () => {
  ...
  it("Sélectionner Espagne (Captain) → Cap-Vert grisé", () =>
    expect(nationsGrisees(["espagne"], matches)).toContain("cap_vert"))
  ...
  it("Cap-Vert est adversaire d'Espagne → estNationAdverse = true", () =>
    expect(estNationAdverse("cap_vert", ["espagne"], matches)).toBe(true))

  it("Allemagne n'est pas adversaire d'Espagne → estNationAdverse = false", () =>
    expect(estNationAdverse("allemagne", ["espagne"], matches)).toBe(false))
  ...
  it("Aucune sélection → aucune nation grisée", () =>
    expect(nationsGrisees([], matches)).toHaveLength(0))
})
...
describe("trierClassement / departagementBoost", () => {
  ...
  it("tableau vide → tableau vide", () =>
    expect(trierClassement([])).toHaveLength(0))
})
