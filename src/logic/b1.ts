import type { EvenementMatch, JoueurLigne, PosteNavigoal } from "./impulse"
  ...
  switch (poste) {
    case "Captain": {
      ...
      for (const j of eligibles) {
        const buts = evenements.filter(
          e => e.joueurId === j.joueurId &&
               e.type === "but" &&
               e.phaseJeu !== "tirs_au_but" &&
               e.officiel,
        ).length
        if (buts > maxButs) { maxButs = buts; best = j.joueurId }
      }
    ...
    case "Second": {
      ...
      for (const j of eligibles) {
        ...
        if (events > maxEvents) { maxEvents = events; best = j.joueurId }
      }
    ...
    case "Navigator": {
      ...
      let maxCreations = 0
      for (const j of eligibles) {
        const creations = evenements.filter(
          e => e.joueurId === j.joueurId &&
               (e.type === "assist" || e.type === "pre_assist") &&
               e.phaseJeu !== "tirs_au_but" &&
               e.officiel,
        ).length
        if (creations > maxCreations) { maxCreations = creations; best = j.joueurId }
      }
    ...
    case "Watch": {
      ...
      let maxMin = 0
      for (const j of eligibles) {
        if (j.minutesJouees > maxMin) { maxMin = j.minutesJouees; best = j.joueurId }
      }
      return best
    }
    ...
    case "Keeper": {
      ...
      for (const j of eligibles) {
        const arrets = evenements.filter(
          e => e.joueurId === j.joueurId && e.type === "arret" && e.officiel,
        ).length
        if (arrets > maxArrets) { maxArrets = arrets; best = j.joueurId }
      }
