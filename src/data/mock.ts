import { calculerPalier, type Palier } from '../logic/palier'
import type { PosteNavigoal } from '../logic/impulse'
...
export interface JourneeCourante {
  ...
  heure_gel_utc: Date           // = heure_premier_match_utc − 15 min (R7)
}
...
export const LIGNE_PAR_POSTE: Record<PosteNavigoal, PosteReel> = {
  Captain: 'FWD',
  Second: 'MID',
  Navigator: 'MID',
  Watch: 'DEF',
...
export const KICKOFFS: Date[] = MATCHS_JOUR.map(m => m.heureKickoffUtc)

const cotesRaw: { nationId: string; valeur: number }[] = [
...
export const COTES: CoteNation[] = cotesRaw.map(c => ({
  ...c,
...
export function coteByNationId(nationId: string): CoteNation | undefined {
  return COTES.find(c => c.nationId === nationId)
}

export function joueursByNationAndPoste(nationId: string, posteReel: PosteReel): Joueur[] {
  return JOUEURS.filter(j => j.nationId === nationId && j.posteReel === posteReel)
}

export const EMPTY_CREW: Crew = {
  Captain: null,
  Second: null,
  Navigator: null,
  Watch: null,
...
const API_TEAM_ID_TO_NATION_ID: Record<number, string> = {}

const API_POS_TO_POSTE_REEL: Record<string, PosteReel> = {
...
export async function loadMatchsJour(date: string): Promise<MatchJour[]> {
  if (!USE_REAL_API) return MATCHS_JOUR
  try {
...
export async function loadJoueurs(fixtureIds: number[]): Promise<Joueur[]> {
  if (!USE_REAL_API) return JOUEURS
  try {
